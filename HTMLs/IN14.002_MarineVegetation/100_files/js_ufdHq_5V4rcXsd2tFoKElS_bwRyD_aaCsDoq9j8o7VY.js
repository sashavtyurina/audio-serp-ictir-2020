...
(function($, Drupal) {
  ...
  Drupal.searchAutocomplete.init = function() {
    ...
    Drupal.searchAutocomplete.prepareList();
  };

  Drupal.searchAutocomplete.prepareSearchField = function() {
    $('#search-form #edit-text').keyup(function(event) {
      var code = (event.keyCode ? event.keyCode : event.which);
      var termsList = $('.search-autocomplete-items');
      // down arrow || up arrow
      if (code == 40 || code == 38) {
        var autocomleteItems = termsList.find('.search-item');
        var currentIndex = -1;
        if (termsList.find('.search-item.hover').length) {
          currentIndex = termsList.find('.search-item.hover').index();
        }
        var newIndex;
        var increment = 1;
        if ((code == 40 && currentIndex === 0) || (code == 38 && currentIndex === 2)) {
          increment = 2;
        }
        ...
        if (selectedItem.offset() && termsList.offset()) {
      ...
      if (code == 13 && termsList.hasClass('active') && termsList.find('.search-autocomplete-item.hover').length) {
        ...
        var element = termsList.find('.search-autocomplete-item.hover');
        if (element && element.hasClass('search-autocomplete-suggestion')) {
          $('#search_icon').click();
          return;
        }

        window.location.href = decodeURI(termsList.find('.search-autocomplete-item.hover').attr('href'));
        return false;
      }
    });

    $('#search-form #edit-text').on('focusout', function() {
      if ($('#search-autocomplete-terms').hasClass('active')) {
        $('#search-autocomplete-terms').removeClass('active');
      }
    });
  }

  Drupal.searchAutocomplete.prepareList = function() {
    var listWrapper = $('#search-autocomplete-terms');

    $('#search-form #edit-text').on('input', function(event) {
      var searchText = event.target.value;

      $.ajax({
        url: '/' + Drupal.settings.pathPrefix + 'search/terms/' + searchText,
        dataType: 'json',
        success: function(data) {
          ...
          if (pathname != 'search') {
            pathname = '/' + Drupal.settings.pathPrefix + 'search/?text=' + searchText;
          }
          else {
            pathname = '?text=' + searchText;
          }

          var search_item = '<a href="' + pathname + '" class="search-autocomplete-suggestion search-autocomplete-item search-item">' + searchText +
            '<span class="help-text">' + Drupal.t('Search all documents with this phrase') + '</span></a>';

          listWrapper.append(search_item);

          var delimiter = '<div class="col-sm-12 search-delimiter search-item">' +
            '<div class="col-sm-5 search-delimiter-text">' + Drupal.t('Matched glossary terms:') +'</div>' +
            '<div class="col-sm-7 search-delimiter-line"><hr></div>' +
            '</div>';

          listWrapper.append(delimiter);

          if (data && data.length) {
            listWrapper.addClass('active');
            $.each(data, function (index, item) {
              var suggestion = '<a href="' + item.url + '" class="search-autocomplete-item search-item">' + item.name
                + '<span class="help-text">' + Drupal.t('See definition and related content') + '</span></a>';

              listWrapper.append(suggestion);
            });
          }
          else {
...
;
jQuery(document).ready(function ($) {
  $("a.js-anchor").once('horizontalTabs').each(function () {
    var scroll_to = $(this).attr('anchor-div');
  ...
  $('.horizontal-tabs-wrapper').each(function () {
    var children = $(this).find('.informea-switcher_link');
    if (children.length === 0) {
      $(this).remove();
    }
  });

  $('.informea-switcher_wrapper').each(function () {
    var active_children = $(this).find('.informea-switcher_link.active');
